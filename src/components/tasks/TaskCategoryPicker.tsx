...
export function TaskCategoryPicker({
  ...
  const { data: categories, loading, create } = useTaskCategories()
  const [open, setOpen] = useState(false)
  const [newName, setNewName] = useState('')
  const [saving, setSaving] = useState(false)
  const canCreate = allowCreate && isManagerOrAdmin(profile)
  const selectedLabel = value ? getTaskCategoryName(categories, value) : allLabel ?? 'Choose category'
      ...
      {open ? createPortal(
        ...
        <div className="w-[min(420px,calc(100vw-2rem))] animate-[modalIn_.16s_ease-out] overflow-hidden rounded-3xl border border-border bg-surface shadow-2xl shadow-text/15">
          <div className="border-b border-border px-4 py-3">
            <p className="text-sm font-semibold text-text">Choose category</p>
          </div>
          ...
          {canCreate ? (
            <div className="border-t border-border bg-background/60 p-3">
              <p className="mb-2 text-xs font-semibold text-muted">Add category</p>
