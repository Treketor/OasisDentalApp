...
create or replace function public.keep_one_default_saved_task_view()
returns trigger
language plpgsql
security definer
...
drop trigger if exists saved_task_views_keep_one_default on public.saved_task_views;
create trigger saved_task_views_keep_one_default
after insert or update of is_default on public.saved_task_views
for each row execute function public.keep_one_default_saved_task_view();
